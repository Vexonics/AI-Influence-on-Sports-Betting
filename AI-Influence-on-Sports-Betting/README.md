...
This project analyzes how AI tools are discussed in the r/sportsbetting Reddit community. Posts were collected via the Arctic Shift API, filtered for AI mentions, topic-modeled, and scored for sentiment using VADER.
...
Both identified topics skewed positive, with the small MLB-stats-adjacent cluster showing the strongest positive sentiment. The larger AI-tools/predictions cluster, while still net positive, showed more spread across sentiment categories (91 positive, 31 neutral, 13 negative), suggesting more mixed reactions to AI betting tools specifically compared to the broader positive baseline.
...
- Topic modeling produced only 2 clusters plus outliers; the larger cluster (Topic 1) is broad and may encompass more than one underlying theme that finer-grained clustering could separate.
